...
package Generic_Router is
   ...
   task type Router_Task (Task_Id  : Router_Range := Draw_Id) is
      ...
      -- entry for rip-chart inter-router communication
      entry Send      (Message : Messages_Router);
      -- entry for inter-router mailing the message send by the framework
      entry Send_Mail (Mail   : Messages_Mail);
